...
- **Dead-code guardrail = knip** (`knip.json`, 6.x): `pnpm knip` locally; CI runs it in `verify` (biome → typecheck → **knip** → test → build) → **unused files / exports / dependencies fail the PR**, so review never has to guess "is this still used?". When knip flags an export, resolve it — delete if fully unused, or drop the `export` keyword if it's only used within its own file (don't add a bogus consumer to silence it). `knip.json` only silences **structural false positives** knip can't see statically: the `cloudflare:workers`/`cloudflare:test` virtual modules (not npm packages), `tailwindcss` referenced from CSS `@import`/`@plugin`, `wrangler` used only by Miniflare/vitest-pool-workers, and `cloudflared` (a brew-installed system binary that `scripts/dev-tunnel.mjs` spawns — it can't be an npm dep). **Never add a real dead export/file/dep to `ignoreDependencies`/`ignore` to dodge a red CI** — that defeats the guardrail; fix the code instead.
