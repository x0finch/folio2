...
**24h 盈亏(24h Gain)**(ADR 0050,取代 0040):
每一行右侧、以及 hero 那行小字上的增量数字 —— **现在的值 − 24 小时前的值**,全站一个口径(百分比基数 = 24 小时前的值)。**充值、提现体现在当天盈亏里,这是用户裁定的设计,不是 bug。** 窗口是**滚动 24 小时**,不是自然日。起点 = 24 小时前那一刻**或更早**的最近一张快照;账户不满 24 小时 → `—`(null),绝不拿首张快照冒充基准;服务停摆过就用手头最近的起点(跨度更长但真实,自己会校正);起点值为 0 → 百分比 null。manual 账户的起点由账本折算(它不写快照,ADR 0018)。DeFi 协议行同一口径,分母 = 起点净值(总敞口分母已废止)。
_Avoid_: TWR / 分段(ADR 0040 的旧口径,已废止)、「今日盈亏」(暗示自然日)、change24h(那是上游给的市场涨跌幅,只是原料且已不再直接显示)
