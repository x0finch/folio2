...
> **「过滤在客户端」那一条已被取代**:见 [ADR 0047](0047-scoping-happens-on-the-server.md) —— 账户域改为服务端按组合筛(代价是切组合要重拉),本 ADR 其余部分不变。
...
- **UI(Phase 1)—— 一个全局顶部选择器,渐进式显示** —— 主页 / 账户页 / Insights **共享同一个顶部 Portfolio 选择器**(住 `_authed` 布局层),选谁三页都 scope 到谁:主页 = 该 Portfolio 净值 / 代币 / Perps,账户页 = 该 Portfolio 的账户(**不再有单独 tab**),Insights 同源。**账户页不设 portfolio tab**——选择器即作用域。
  ...
  - **空 Portfolio 持久、不自动删** —— 命名 Portfolio 由「移账户进来」诞生(非空创建),但把成员全移走后**它仍留着**(不自动消失);要去掉就到管理菜单**显式删**(成员若还有则退回默认)。空 Portfolio 的视图 = 净值 $0 + 空账户列表,**不做特殊空态**(自然空着即可)。默认 Portfolio 同样持久、且**不可删**。
