...
**后记:那句「很轻」不全对。** T1 的 e2e 量出来了 —— app 侧的 `db` 是个 Proxy,每次属性访问重建整份门面,而门面在本 ADR 之后还要给每个已迁领域各建一个 per-user 转接器;一个总览请求 23 次属性访问,足以把锁屏那条用例拖成偶发红。按 env 记忆化之后转绿。**过渡形状的开销是能被量出来的,不是纸上代价** —— 这也是它必须有明确死期的理由。

**门面已于 #394 T8 删除**(contract 步)。`@folio/db` 的出口现在只有 per-user 的 Tag + layer,「每次调用各装一次」这条路在类型上不存在了,而不是靠人记得别用。唯一还按调用逐次装的是 `@folio/sync` 的 `SyncDeps`(它的方法各收一个 userId,cron 用一份 deps 扫全部用户,与 per-user 装配对不上)—— 那笔开销现在**写在调用点上**(`runDbStore`),不再藏在门面后面;真要收掉得让 sync 的服务也变成 per-user,是它自己的一票。
