...
Status: accepted(决策 #4「可切换多价格源」已被 [ADR 0014](0014-drop-runtime-price-source-switching.md) 取代;合包 #1 与 vendor 中立身份 #2/#3 仍有效)— 激活并扩展 [issue #46](https://github.com/x0finch/folio2/issues/46)(代币源 vendor 中立)。合并 [ADR 0005](0005-shared-coingecko-client-and-platforms-package.md)(独立 `@folio/platforms`)与 [ADR 0006](0006-multi-currency-display.md)(独立 `@folio/fx`)的**打包边界**——两者的领域决策(平台元数据缓存、存 USD/展示层换算)不变,只是合入统一门面。聚合原则([ADR 0001](0001-aggregate-by-token-group.md) 按 TokenGroup、[ADR 0002](0002-never-merge-by-symbol.md) 永不按 symbol 归并)不变,归并键改由内部 id 承载。经 `grill-with-docs` 定案(决策记录见 `.scratch/plans/oracle-grill-decisions.md`,一次性)。Phase 1(合包 + 身份)已实现(#71/#72/#73);Phase 3 的多源定价 + 估值 policy **运行时行为**见 [ADR 0013](0013-multi-source-pricing-and-valuation-policy.md)。决策 #2/#3(内部 id 为唯一身份)由 [ADR 0021](0021-per-user-tokens-token-id-as-sole-identity.md) **强化并加上 per-user 维度**:`tokens` 收归 user 私有,`tokens.id` 成为系统内部唯一贯穿身份,`token_vendor_ids` 并入 `token_refs` 成为「任何名字 → 我的 token id」的单一映射桥。 「多源」经 [ADR 0023](0023-oracle-layering-swappable-source.md) 收紧:从身份中立推进到**依赖表层面**中立(source 与 store 一样注入,服务层不依赖任何 source 包)。
