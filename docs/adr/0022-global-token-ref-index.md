...
- **已实测**(2026-07-26,在真 workerd isolate 里跑整条刷表路径,不是本机 curl):响应 **2.63 MB**、**17,841** 个币、`/asset_platforms` 列 **461** 条链、产出 **23,004 行**(跳过 1,530 条残缺条目),链对照**零失配**(显式的非 EVM slug 表是全的)。
  分段耗时:网络 1,861ms(I/O)· `JSON.parse` **27ms CPU** · 「两个端点 → 映射行」的纯转换 **22ms CPU** · D1 写 636ms · 合计 2.5s。**几 MB JSON 的 parse 只花 ~50ms CPU** —— 对 scheduled handler 的预算完全不是问题,当初写下这条待测项时担心的那点不成立。
  据此定批大小:20 行/语句(80 个绑定参数,稳在 D1 的 100 上限内)× 50 语句/批 = 1000 行/批 → **24 批**,实测 636ms 写完。
