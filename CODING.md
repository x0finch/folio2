...
**共用不是新建文件的理由。** 两处要用同一段逻辑时,先问它属于哪个概念,放进那个概念已经住着的文件 —— 别为三行规则开一个共享文件,那只是把一件事拆成两处,文件树越来越细碎而没人多懂一点。判据还是上一句:文件得有自己要讲的事,「两边都要用」不算。跨层同理 —— 纯模块被两边 import 就够了(那时在文件里写明它必须保持纯净:一旦有人加 server-only 的 import,另一侧就炸),不必为了「看起来像共享的」把它搬进公共目录。
