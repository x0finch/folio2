...
//
// 关键:registry 不手写映射表,而是由各 provider 的 accountType 字段【自动组装】。
// 于是 "provider 服务哪个 type" 只在 provider 自身声明一次(单一事实源),
// 新增 provider 只需把它加进 provider 列表,不可能写错或漏登记 key。
//
// 用 Partial:分阶段开发时尚未实现的 type 允许缺席;getProvider 在运行时兜底报错。
