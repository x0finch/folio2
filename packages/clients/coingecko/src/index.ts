// @folio/coingecko-client —— 共享 CoinGecko HTTP 客户端(tokens + platforms 复用)。
// 薄 IO:拼 URL/头 + 错误映射,解析交调用方。含两处 CF Workers 修复:
...
export interface CoinGeckoClient {
  /** GET path(+query);404 且 notFoundAsNull 时返回 null,其余非 2xx / 网络 / 坏 JSON 抛 CoinGeckoError。 */
  request(
    path: string,
