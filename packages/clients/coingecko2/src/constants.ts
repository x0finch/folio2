...
//
// **重试的那三个常量没搬过来**(`CG_RETRY_ATTEMPTS` / `CG_RETRY_BASE_MS` / `CG_RETRY_MAX_WAIT_MS`)——
// 这一版的 client **不自带重试**,与另外七个一致:重试由调用方 `Effect.retry(策略)` 加在外面。
// 见 index.ts 的说明,以及 `@folio/sync` 里那份已有的 `Schedule`。
