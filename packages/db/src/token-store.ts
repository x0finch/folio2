...
export function createTokenStore(env: DbEnv, opts: TokenStoreOpts): TokenStore {
  ...
  const mk = (vendorId: string): TokenRef => tokenRef.issued(source, vendorId);
  ...
  return {
    ...
    async putWarm(rows, warmTtlMs, infoTtlMs) {
      ...
      for (const { info, price } of rows) {
        ...
        stmts.push(
          ...
          vendorMapUpsert(id, vid(info.ref)),
          db
            .insert(tokenIndex)
    ...
    async linkTokenRefToCgk(key, info, price, ttls) {
      ...
      const mapped = await db
        ...
        .from(tokenVendorIds)
        .where(and(eq(tokenVendorIds.vendor, source), eq(tokenVendorIds.vendorId, vid(info.ref))));
    ...
    async putPrices(prices, ttlMs) {
      ...
      const ids = await existingIds(prices.map((p) => vid(p.ref)));
      const stmts: Stmt[] = [];
      for (const p of prices) {
        const id = ids.get(vid(p.ref));
        if (!id) continue;
        stmts.push(
          db
            .update(tokens)
