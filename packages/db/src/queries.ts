...
import type { BalanceKind, DetailBlock } from "@folio/connectors-basic";
import {
  and,
  asc,
  desc,
...
// D1 每条 SQL 最多 100 个绑定参数;snapshot_balances 每行 9 列 → 每块 11 行(99 参数,压在上限内)。
...
export interface SnapshotBalanceInput {
  ...
  detail?: DetailBlock[]; // provider 专属展示块(ADR 0010),JSON.stringify → detail_json 落库
}
...
export async function writeSnapshot(
  ...
  const balanceRows = input.balances.map((b) => ({
    ...
    detailJson: b.detail && b.detail.length > 0 ? JSON.stringify(b.detail) : null,
  }));
