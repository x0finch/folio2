...
// —— 批大小按实测定(#199 的验收项;2026-07-26 在真 workerd isolate 里跑整条刷表路径)——
//
//   响应 2.63 MB · 17,841 个币 · `/asset_platforms` 列 461 条链 · 产出 **23,004 行**(跳过 1,530 条残缺条目)
//   分段耗时:网络 1,861ms(I/O)· `JSON.parse` **27ms CPU** · 纯转换 **22ms CPU** · D1 写 636ms · 合计 2.5s
//   链对照零失配(`unmatchedPlatforms` 为空)—— 显式的非 EVM slug 表是全的。
//
