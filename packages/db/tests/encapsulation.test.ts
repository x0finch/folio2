...
describe("@folio/db encapsulation", () => {
  ...
  // 出口是**per-user 的服务**(ADR 0037):一个名字同时是 Tag 和它的 layer 工厂
  // (`Effect.Service`,#501),方法签名里没有 userId。
  ...
  it("exposes per-user store services + global infra, not a facade or raw query functions", () => {
    ...
    // `Effect.Service` 出来的是个 class(所以 typeof 是 function),`.Default(userId)` 是它的 layer。
    expect(typeof db.AccountStore).toBe("function");
    expect(typeof db.AccountStore.Default).toBe("function");
