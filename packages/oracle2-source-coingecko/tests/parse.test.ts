...
describe("ref 与 coin id 的双向", () => {
  it("造:coin id 规范为小写 kebab,命名者恒为本 adapter 的 id", () => {
    expect(cgkRef("USD-Coin")).toBe(`${SOURCE_ID}/usd-coin`);
  });

  it("取:本源命名的 ref → coin id;别家 / 链上寻址 → undefined", () => {
    expect(coinIdOf(`${SOURCE_ID}/bitcoin`)).toBe("bitcoin");
...
describe("parseSearch / parseSimplePrice / parseContract / parsePriceSeries", () => {
  it("search:name 缺则回退 symbol;取 large 优先", () => {
    ...
    expect(out).toEqual([
      { ref: `${SOURCE_ID}/bitcoin`, symbol: "btc", name: "btc", logo: "l.png" },
    ]);
  });
  ...
  it("simple/price:按 ref 索引;缺 usd 的条目跳过;有 last_updated_at 则用它(秒→毫秒)", () => {
    ...
    expect(out.get(`${SOURCE_ID}/bitcoin`)).toEqual({
      unitPrice: 60000,
      change24h: 1.5,
      asOf: 1_700_000_000_000,
    });
    expect(out.has(`${SOURCE_ID}/broken`)).toBe(false);
  });
  ...
  it("contract:无 id → null;无价 → 只出元信息", () => {
    ...
    expect(out).toMatchObject({ ref: `${SOURCE_ID}/usd-coin`, logo: "s.png" });
