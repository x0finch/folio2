...
import { entries, makeCoinstats } from "../src";
...
describe("coinstats factory (方案 A multi-type)", () => {
  it("one entry per configured type", () => {
    const types = entries.map((e) => e.manifest.accountType).sort();
  ...
  it("binds the right connectionId per type", async () => {
    ...
    const sui = makeCoinstats("sui-wallet", "k");
...
describe("coinstats fetchBalances", () => {
  const provider = makeCoinstats("solana", "k");
  ...
  it("throws INVALID_CREDENTIALS when the global key is missing (no request)", async () => {
    ...
    const keyless = makeCoinstats("solana");
