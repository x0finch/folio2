...
// 一个 manual 账户 = 一个手记资产;持仓走 ctx.creds(symbol/amount/usdValue,P6.6.2)。
...
describe("customProvider.fetchBalances", () => {
  it("maps the single manual holding to one Balance (kind=manual)", async () => {
    const balances = await customProvider.fetchBalances(
      ctx({ symbol: "BTC", amount: 0.5, usdValue: 32000 }),
    );
