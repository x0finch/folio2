...
describe("三种键", () => {
  it("只有 warm / fx:<币种> / platform:<键> 三种", async () => {
    ...
    await warmRows(cache, upstream, 10, cache.now);
  ...
  it("TTL 语义:warm 短、平台名图长", async () => {
    const cache = fakeCacheStore();
    await warmRows(cache, fakeUpstream(), 10, cache.now);
...
describe("warm 走 SWR,一次整份写", () => {
  it("三个币一次写;没过期就不拉上游", async () => {
    ...
    expect(await warmRows(cache, upstream, 50, cache.now)).toHaveLength(3);
    expect(cache.writes).toBe(1); // 三个币,一次写
    expect(cache.entries.size).toBe(1);

    await warmRows(cache, upstream, 50, cache.now);
  ...
  it("上游挂了 → 给旧的那份,不抛(SWR 兜的)", async () => {
    ...
    expect(await warmRows(cache, upstream, 50, cache.now)).toHaveLength(1);
  });
});
...
describe("排行榜与 symbol 候选出自同一份 rows", () => {
  it("取前 N 名按市值升序,无 rank 者垫底", async () => {
    ...
    const rows = await warmRows(cache, upstream, 50, cache.now);
