...
function stubFetch(routes: Record<string, unknown | (() => unknown)>) {
  ...
  vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
    ...
    if (body instanceof Error) return new Response(null, { status: 429 });
...
describe("fetchMarkets 分页", () => {
  ...
  it("单页装得下 → 只请求一次", async () => {
    ...
    expect(await createCoinGeckoUpstream().fetchMarkets({ topN: 10 })).toHaveLength(10);
  ...
  it("上游给不满一页 → 提前收手,不问下一页", async () => {
    ...
    const rows = await createCoinGeckoUpstream().fetchMarkets({ topN: MARKETS_PER_PAGE * 3 });
...
describe("链标识 → CoinGecko 的 asset_platform", () => {
  ...
  it("EVM 靠数字 chainId 对齐(比 slug 可靠)", async () => {
    ...
    const got = await createCoinGeckoUpstream().fetchByContract("42161", "0xAF88");
  ...
  it("非 EVM 给 slug,大小写不敏感", async () => {
    const calls = stubFetch({ "/asset_platforms": PLATFORMS, ...CONTRACT });
    await createCoinGeckoUpstream().fetchByContract("Solana", "EPjF");
  ...
  it("平台表里没有这条链 → null,连合约端点都不问", async () => {
    const calls = stubFetch({ "/asset_platforms": PLATFORMS, ...CONTRACT });
    expect(await createCoinGeckoUpstream().fetchByContract("evm:99999", "0xabc")).toBeNull();
    expect(calls.some((c) => c.path.includes("/contract/"))).toBe(false);
  });

  it("缺 id 的平台条目跳过,不落一个 undefined 键", async () => {
    const calls = stubFetch({ "/asset_platforms": PLATFORMS, ...CONTRACT });
    expect(await createCoinGeckoUpstream().fetchByContract("999", "0xabc")).toBeNull();
    expect(calls.some((c) => c.path.includes("/contract/"))).toBe(false);
  });
});
...
describe("平台表记忆", () => {
  it("一次 sync 里连查几个合约只拉一次平台表", async () => {
    ...
    const upstream = createCoinGeckoUpstream();
  ...
  it("平台表拉失败 → 不记住,下一次重新拉并成功", async () => {
    let attempt = 0;
    stubFetch({
      "/asset_platforms": () => (++attempt === 1 ? new Error("429") : PLATFORMS),
      "/contract/": { id: "usd-coin", symbol: "usdc", name: "USD Coin" },
    });
    const upstream = createCoinGeckoUpstream();
...
describe("不是本源命名的 ref → 不发请求", () => {
  ...
  it("历史价:链上寻址的 ref 本源给不出 → 空,不发请求", async () => {
    ...
    expect(await createCoinGeckoUpstream().fetchPriceSeries("evm:1/contract:0xa0b8", 0, 1)).toEqual(
      [],
    );
    expect(calls).toHaveLength(0);
  });
...
describe("搜索与全局映射", () => {
  ...
  it("全局映射一次并发拉两个端点(币目录 + 平台表)", async () => {
    ...
    const got = await createCoinGeckoUpstream().fetchRefIndex();
