...
import { createDefiLlamaSource } from "../src/price";
...
// mock 全局 fetch(镜像 coingecko source 测试),记录调用便于断言 url/headers。afterEach 还原。
...
describe("DefiLlama error mapping", () => {
  it("429 → RATE_LIMITED (retryable) with retryAfterMs", async () => {
    ...
    await expect(
      createDefiLlamaSource().fetchPrices([dl("coingecko:bitcoin")]),
  ...
  it("always sends a User-Agent header", async () => {
    ...
    await createDefiLlamaSource().fetchPrices([dl("coingecko:bitcoin")]);
...
describe("fetchPrices", () => {
  it("joins defillama coin keys into the path and parses", async () => {
    ...
    const prices = await createDefiLlamaSource().fetchPrices([
  ...
  it("ignores non-defillama refs; all-filtered → empty map, no request", async () => {
    ...
    const out = await createDefiLlamaSource().fetchPrices([cg]);
...
describe("fetchByContract", () => {
  it("translates chain→slug, builds {slug}:{addr} key, parses coin", async () => {
    ...
    const out = await createDefiLlamaSource().fetchByContract(
      "ethereum",
  ...
  it("unknown chain → lowercased slug fallback", async () => {
    const { calls } = mockFetch(() => ({ body: { coins: {} } }));
    await createDefiLlamaSource().fetchByContract("SomeChain", "0xAbC");
    expect(calls[0].url.pathname).toBe("/prices/current/somechain:0xabc");
  });

  it("coin absent in response → null", async () => {
    mockFetch(() => ({ body: { coins: {} } }));
    const out = await createDefiLlamaSource().fetchByContract("ethereum", "0xdead");
    expect(out).toBeNull();
  });
});
...
describe("PriceSource shape", () => {
  it("source tag = defillama", () => {
    expect(createDefiLlamaSource().source).toBe("defillama");
  });
});
