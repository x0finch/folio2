...
describe("CoinGeckoSource error mapping", () => {
  it("429 → RATE_LIMITED (retryable) with retryAfterMs from header", async () => {
    ...
    const src = createCoinGeckoSource(NO_WAIT);
...
describe("CoinGeckoSource config", () => {
  it("free base + demo header when key given", async () => {
    ...
    await createCoinGeckoSource({ ...NO_WAIT, apiKey: "k" }).fetchPrices([cg("bitcoin")]);
  ...
  it("pro base + pro header when pro", async () => {
    const { calls } = mockFetch(() => ({ body: {} }));
    await createCoinGeckoSource({ ...NO_WAIT, apiKey: "k", pro: true }).fetchPrices([
      cg("bitcoin"),
    ]);
  ...
  it("always sends a User-Agent header (keyless too)", async () => {
    const { calls } = mockFetch(() => ({ body: {} }));
    await createCoinGeckoSource(NO_WAIT).fetchPrices([cg("bitcoin")]);
...
describe("fetchPriceSeries", () => {
  ...
  it("drops malformed pairs (short tuple / non-finite)", async () => {
    ...
    const out = await createCoinGeckoSource(NO_WAIT).fetchPriceSeries(cg("bitcoin"), 1, 2);
  ...
  it("upstream error → TokenError (mapped, not raw)", async () => {
    mockFetch(() => ({ status: 503 }));
    await expect(
      createCoinGeckoSource(NO_WAIT).fetchPriceSeries(cg("bitcoin"), 1, 2),
...
describe("searchTokens", () => {
  it("sends query and parses coins[] → TokenInfo[]", async () => {
    ...
    const out = await createCoinGeckoSource(NO_WAIT).searchTokens("btc");
  ...
  it("blank query → [] without a request", async () => {
    ...
    expect(await createCoinGeckoSource(NO_WAIT).searchTokens("  ")).toEqual([]);
    expect(calls).toHaveLength(0);
  });
});
...
describe("fetchByContract", () => {
  it("maps chain→platform internally, then resolves; 200 → {ref, info, price}", async () => {
    ...
    const out = await createCoinGeckoSource(NO_WAIT).fetchByContract("ethereum", "0xABC");
  ...
  it("translates our chain slug to CGK platform slug (polygon → polygon-pos)", async () => {
    const { calls } = contractMock();
    await createCoinGeckoSource(NO_WAIT).fetchByContract("polygon-pos", "0xABC");
  ...
  it("chain not in asset_platforms → null (no per-contract call)", async () => {
    const { calls } = contractMock();
    const out = await createCoinGeckoSource(NO_WAIT).fetchByContract("unknownchain", "0x1");
    expect(out).toBeNull();
  ...
  it("per-contract 404 → null (not thrown)", async () => {
    ...
    const out = await createCoinGeckoSource(NO_WAIT).fetchByContract("ethereum", "0xdead");
    expect(out).toBeNull();
  });
});
