...
describe("toRefIndexRows —— 纯转换(golden)", () => {
  ...
  it("每一行都是 `contract:` 形 —— 这张表里全是链上合约(ADR 0020 第三轮)", () => {
    expect(result.rows.every((r) => r.ref.includes("/contract:"))).toBe(true);
  });
  ...
  it("EVM 平台翻成 evm:<chainId>,地址小写归一", () => {
    expect(byRef.get("evm:1/contract:0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48")).toBe("usd-coin");
    expect(byRef.get("evm:42161/contract:0xaf88d065e77c8cc2239327c5edb3a432268e5831")).toBe(
      "usd-coin",
    );
    expect(byRef.get("evm:8453/contract:0x833589fcd6edb6e08f4c7c32d4f71b54bda02913")).toBe(
      "usd-coin",
    );
  });

  it("非 EVM 走显式对照,地址原样保留(base58 / bech32 大小写敏感)", () => {
    expect(byRef.get("solana/contract:EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v")).toBe(
      "usd-coin",
    );
    expect(byRef.get("cosmos/contract:uatom")).toBe("cosmos");
    expect(byRef.get("sui/contract:0xSuiTokenAddress00000000000000000000000000AB")).toBe("suicoin");
  });
  ...
  it("残缺条目一律跳过:没 id 的币、null / 空白地址、没有 platforms 键", () => {
    expect(byRef.has("evm:1/contract:0xbeef")).toBe(false);
