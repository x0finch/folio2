...
// 三条查找路径:代币表主键 / (source, identifier) 唯一索引(cgk id)/ tokenRef(索引表)。
// key 归一(symbol 大写、tokenRef 构造)由调用方(entry 的 normalize)完成,store 只按 key 存/查。
export interface TokenStore {
  ...
  ensureTokenRef(key: string, seed: ProviderTokenSeed, indexTtlMs: number): Promise<void>;
  ...
  // 代理 URL 却在此 404(getByTokenRef 渲染路径也不门控 info)。
