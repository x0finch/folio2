...
// TokenProvider = 两面之交,供代币服务(createTokens)消费完整体;单能力源(如 DefiLlama)只实现 PriceSource。
// 「供哪一面」由 vendor 在注册表(VendorImpl)里挂哪个工厂决定 —— 路由据字段在场与否判定,不另存能力集。
// 通用层只说 `chain`(我们的链标识);各 provider 内部把 chain 翻译成自己的寻址命名,不外泄。价格一律 USD。
...
export interface PriceSource {
  // 本 provider 产出的 `TokenRef` 的 source 标签。
  readonly source: TokenRef["source"];
...
export type TokenProvider = TokenMetaSource & PriceSource;
