...
//
// ADR 0020 第三、四轮之后它们判的是 `issued` 这一支 —— 只有「厂商命名」才是 (vendor, id);
// 链上合约与原生币都不是,必须拆不出来。
//
...
// `issued`;哪天那道守卫被挪走或换写法,得有东西接着拦,而不是静默拆出一段 `vendor="evm:1"`。

describe("cgkRef", () => {
  it("coin id 规范为小写 kebab,归一在生产者这一侧做", () => {
    expect(cgkRef("USD-Coin")).toBe(`${CGK_VENDOR}/issued:usd-coin`);
  });
});
...
describe("vendorPartsOf", () => {
  it("厂商命名 → 存储层的两列", () => {
    expect(vendorPartsOf("coingecko/issued:usd-coin")).toEqual({
      vendor: "coingecko",
      vendorId: "usd-coin",
    });
    // 场馆命名也是 issued 形 —— 它确实是「binance 管这个币叫 USDC」。
    expect(vendorPartsOf("binance/issued:USDC")).toEqual({ vendor: "binance", vendorId: "USDC" });
  });
