...
// available/locked → keyValue detail 块(仅当有锁仓分量且可估值时才吐,避免噪音)。
// 值用 USD(free/locked × price,与该行的现货估值同口径)+ format:"usd" → 前端跟随显示币种/双语渲染。
// 全部可用(locked=0)或无价(price 缺)则不吐块。
function lockedBlocks(free: number, locked: number, price: number | undefined): DetailBlock[] {
  if (!(locked > 0) || price == null) return [];
  return [
    {
      type: "keyValue",
      label: DETAIL_LABEL.breakdown,
      items: [
        { label: DETAIL_LABEL.available, value: free * price, format: "usd" },
        { label: DETAIL_LABEL.locked, value: locked * price, format: "usd" },
      ],
    },
  ];
}
...
// 有锁仓分量时挂 keyValue detail 块展示 available/locked(USD 口径,见 lockedBlocks)。
export function parseAccountBalances(
  ...
  for (const b of account.balances ?? []) {
    ...
    const detail = lockedBlocks(free, locked, price);
