...
//     与被测行为无关。
//
// 这里不造「24 小时前」的数据:新建的手记账户,活动就发生在**此刻**,窗口内没有 24 小时前的起点。
// **FOL-51 用户裁定**(推翻 FOL-43 原「账户不满 24h 显 —」):账户级把「今天新建/充值」当 `new` ——
// 起点 0、`endpointGain(0, 现值)` = **全额算涨**(视同充值,与组合级一致)。2 BTC @ 50000 = $100,000,
// 起点 0 → 增量 `+$100,000.00`(pct=null,分母 0 → 不显百分比)。
// 而「有增量」与「整行不渲染」(漏传 prop → `undefined` → 那一格整个消失)在界面上必须分得开。
//
// 口径变迁:旧 ADR 0040 TWR 下活动就在此刻 → 分段盈亏 0 → `$0.00 0.00%`;FOL-51 两端相减 + `new`
// 计入 → `+$100,000.00`。断言文本随之更新。

test.describe("手记账户的 24h 盈亏", () => {
  test.describe.configure({ timeout: 120_000 });

  test("Tokens 那行渲染出增量(+现值),而不是整行不见", async ({ page }) => {
    ...
    // —— 断言那一行确实带着增量 ——
    // 今天新建 = `new` → 起点 0 → 增量 `+$100,000.00`(pct=null → 无百分比)。漏传时这一行整个不存在。
    // 这一屏上 `+$100,000.00` 应该出现**恰好三处**,而且都是那个增量格:
    //   ① 账户列表那一行  ② 抽屉头  ③ 抽屉里 Tokens 那一行
    // 三处分别走 <ValueDelta>、抽屉头 SheetHeaderGain、以及 <TokenRowContent>(→ <ValueDelta>)——
    // 正是本次要统一的三条路。**`+` 前缀只属于增量**:市值格显 `$100,000.00`(无 `+`),不会被误数进来。
    // 漏传时第三处整行不渲染、数目掉到 2:这个计数就是那个 bug 的精确形状。
    //
    // `exact: true`:只匹配文本**正好**是 `+$100,000.00` 的元素(增量格),不匹配「碰巧含这串」的别处。
    await expect(page.getByText("+$100,000.00", { exact: true })).toHaveCount(3);
  });
});
