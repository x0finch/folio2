...
//     与被测行为无关。
//
// 这里不需要造「24 小时前」的数据:新建的手记账户,活动就发生在此刻,盈亏算出来正好是 **0**。
// 而 0 与「没有这个数」在界面上必须长得不一样 —— 前者渲染一个 $0.00,后者整行不渲染。
// 这个区分正是 ADR 0040 三态的核心,也正好是漏传时会塌掉的那一格。

test.describe("手记账户的 24h 盈亏", () => {
  test.describe.configure({ timeout: 120_000 });

  test("Tokens 那行渲染出增量(哪怕是 0),而不是整行不见", async ({ page }) => {
    ...
    // —— 断言那一行确实带着增量 ——
    // 活动就发生在此刻 → 盈亏恰好 0 → 渲染 `$0.00`。漏传时这一行整个不存在。
    // 这一屏上同一个数应该出现**三处**,而且都是同一个数:
    //   ① 账户列表那一行  ② 抽屉头  ③ 抽屉里 Tokens 那一行
    // 三处分别走 <ValueDelta>、抽屉头手搓的那块、以及 <TokenRowContent> —— 正是本次要统一的三条路。
    ...
    await expect(page.getByText("$0.00 0.00%")).toHaveCount(3);
  });
});
