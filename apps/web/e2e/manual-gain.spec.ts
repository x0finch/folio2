...
// (`snapshots.asOf` 的下界还没盖到它)→ 两端相减(ADR 0050)算不出 → 三处增量格都渲染 `—`。
// 而 `—`(算不出)与「整行不渲染」(漏传 prop → `undefined`)在界面上必须长得不一样 —— 前者是一格
// `—`,后者那一格整个消失。这个区分正是本用例要挡的那一格。
//
// **旧口径(ADR 0040 TWR)下这里显 `$0.00 0.00%`**(活动就在此刻 → 分段盈亏恰好 0);FOL-51 改两端
// 相减后,此刻新建、没有 24 小时前观测 → 起点空 → 显 `—`。断言文本随之从 `$0.00 0.00%` 改成 `—`。

test.describe("手记账户的 24h 盈亏", () => {
  test.describe.configure({ timeout: 120_000 });

  test("Tokens 那行渲染出增量(哪怕是 `—`),而不是整行不见", async ({ page }) => {
    ...
    // —— 断言那一行确实带着增量 ——
    // 此刻新建、没有 24 小时前的起点 → 两端相减算不出 → 增量格渲染 `—`(不是 $0.00,也不是空)。
    // 漏传时这一行整个不存在。这一屏上 `—` 应该出现**恰好三处**,而且都是这个算不出的增量格:
    //   ① 账户列表那一行  ② 抽屉头  ③ 抽屉里 Tokens 那一行
    // 三处分别走 <ValueDelta>、抽屉头 SheetHeaderGain、以及 <TokenRowContent>(→ <ValueDelta>)——
    // 正是本次要统一的三条路。这一屏上 `NO_VALUE`(`—`)只有这三处会渲染(其余都是市值 / 名称 / 数量,
    // 见各自的 render;best/worst/稳定币占比那几个 `—` 是首页的,不在账户页)。漏传时第三处整行不渲染、
    // 数目掉到 2:这个计数就是那个 bug 的精确形状。
    //
    // `exact: true`:只匹配文本**正好**是 `—` 的元素(算不出的增量格),不会把「碰巧含破折号」的别处误算进来。
