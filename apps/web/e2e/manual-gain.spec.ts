...
// **最终口径(ADR 0050,最简两档)**:有 24h 前基准 → 两端相减;**没有基准就一律 `—`**(新账户 /
// 新建 manual 不满 24h、断线超 7 天,都一样,不硬算)。所以三处增量格都渲染 `—`。
// 而 `—`(算不出)与「整行不渲染」(漏传 prop → `undefined` → 那一格整个消失)在界面上必须分得开。
//
// 口径变迁记档:旧 ADR 0040 TWR 下活动就在此刻 → 分段盈亏 0 → `$0.00 0.00%`;中途一版把新建当「起点 0
// 全额算涨」→ `+$100,000.00`;**最终拍定两档,无基准 → `—`**。断言随之定在 `—` × 3。

test.describe("手记账户的 24h 盈亏", () => {
  test.describe.configure({ timeout: 120_000 });

  test("Tokens 那行渲染出增量(`—`),而不是整行不见", async ({ page }) => {
    ...
    // —— 断言那一行确实带着增量 ——
    // 今天新建、无 24h 前基准 → 增量格渲染 `—`(不是 $0.00,也不是整行消失)。漏传时这一行整个不存在。
    // 这一屏上 `—` 应该出现**恰好三处**,而且都是那个算不出的增量格:
    ...
    // best/worst/稳定币占比那几个 `—` 是首页的,不在账户页)。漏传时第三处整行不渲染、数目掉到 2:
    // 这个计数就是那个 bug 的精确形状。
    //
    // `exact: true`:只匹配文本**正好**是 `—` 的元素(算不出的增量格),不把「碰巧含破折号」的别处误算进来。
