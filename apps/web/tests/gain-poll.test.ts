...
import { gainPollDelay, POLL_INTERVAL } from "@/lib/queries/constants";
...
describe("gainPollDelay", () => {
  it("第一发就是 POLL_INTERVAL.gain —— 正常情况下补算几百毫秒就落地,一发即中", () => {
    expect(gainPollDelay(0)).toBe(POLL_INTERVAL.gain);
  });

  it("越问越慢(退避),而且封顶", () => {
    const delays = [0, 1, 2, 3, 4, 5, 6, 7].map((n) => gainPollDelay(n));
  ...
  it("问够了就**放弃**,不是无限地慢慢问", () => {
    expect(gainPollDelay(8)).toBe(false);
    expect(gainPollDelay(50)).toBe(false);
  });

  it("退避总时长是分钟量级 —— 够跨过一次抖动,不至于一直挂着", () => {
    let total = 0;
    for (let n = 0; ; n++) {
      const d = gainPollDelay(n);
      if (d === false) break;
      total += d;
    }
