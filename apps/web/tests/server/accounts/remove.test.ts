...
describe("removeAccount", () => {
  it("删掉 → 账户与它的快照、余额一并消失", async () => {
    const acc = await seedAccount(USER, "甲", "bitcoin");
    await seedSnapshot(USER, acc.id, Date.now(), [{ tokenId: BTC, amount: 1, usdValue: 100 }]);
    expect(await countRows("accounts", USER)).toBe(1);

    await call(USER, handleRemoveAccount({ accountId: acc.id }));

    expect(await countRows("accounts", USER)).toBe(0);
    expect(await db(USER).snapshots.latest()).toEqual([]);
  });
