...
describe("removeManualToken", () => {
  it("删掉一个币 → 明细里没了,它的活动一并消失", async () => {
    ...
    await call(USER, handleRemoveManualToken({ accountId: acc.id, tokenId: before.tokens[0].id }));
  ...
  it("这个币在别的手记账户也有 → 只删本账户那份", async () => {
    const a = await seedManualAccount(USER, "甲", { symbol: "BTC", unitPrice: 1, amount: 1 });
    const b = await seedManualAccount(USER, "乙", { symbol: "BTC", unitPrice: 1, amount: 3 });
    const aDetail = await detail(USER, a.id);

    await call(USER, handleRemoveManualToken({ accountId: a.id, tokenId: aDetail.tokens[0].id }));

    expect((await detail(USER, a.id)).tokens).toEqual([]);
