...
describe("listAccountHoldings", () => {
  ...
  it("同一账户的两张快照 → 只用最新那张,不把两张相加", async () => {
    const acc = await seedAccount(USER, "甲", "bitcoin");
    await seedSnapshot(USER, acc.id, ago(2 * DAY), [{ tokenId: BTC, amount: 1, usdValue: 100 }]);
    await seedSnapshot(USER, acc.id, NOW, [{ tokenId: BTC, amount: 1, usdValue: 150 }]);

    const view = await call(USER, handleListAccountHoldings());

    expect(view.rows[0].totalUsd).toBe(150);
  });
  ...
  it("别人的账户不出现在我的列表里", async () => {
    ...
    await seedAccount(USER, "我的", "bitcoin");

    const view = await call(USER, handleListAccountHoldings());

    expect(view.rows.map((r) => r.account.id)).not.toContain(theirs.id);
  });
