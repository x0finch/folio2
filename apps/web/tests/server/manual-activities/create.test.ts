...
describe("createManualActivities", () => {
  ...
  it("提交的币是新的 → 新建声明 + 落活动,一次完成", async () => {
    const acc = await seedManualAccount(USER, "手记", { symbol: "BTC", unitPrice: 1, amount: 1 });
    ...
    await call(
      ...
      handleCreateManualActivities({
        accountId: acc.id,
        drafts: [draft("ETH", "add", 4, at(10))],
      }),
    );

    expect(await amountOf(acc.id, "ETH")).toBe(4);
  ...
  it("同一批里先 add 再 reduce、中间量刚好够 → 通过(按顺序算,不是按最终态算)", async () => {
    // 这条与上一条是一对:最终态一样都是 0 净变化,但顺序决定合法性。
    ...
    const out = await call(
      ...
      handleCreateManualActivities({
        accountId: acc.id,
        drafts: [draft("BTC", "add", 5, at(8)), draft("BTC", "reduce", 5, at(9))],
      }),
    );

    expect(out).toEqual({ ok: true });
    expect(await amountOf(acc.id, "BTC")).toBe(0);
  });

  it("顺序反过来(先 reduce 再 add)→ 超支,整批拒", async () => {
    const acc = await seedManualAccount(USER, "手记", { symbol: "BTC", unitPrice: 1, amount: 0 });
    ...
    const out = await call(
      ...
      handleCreateManualActivities({
        accountId: acc.id,
        drafts: [draft("BTC", "reduce", 5, at(8)), draft("BTC", "add", 5, at(9))],
      }),
    );
  ...
  it("往两笔活动中间插一笔更早的 set → 之后每笔按新顺序重算", async () => {
    const acc = await seedManualAccount(USER, "手记", { symbol: "BTC", unitPrice: 1, amount: 0 });
    await call(
      USER,
      handleCreateManualActivities({
        accountId: acc.id,
        drafts: [draft("BTC", "add", 10, at(5)), draft("BTC", "reduce", 6, at(9))],
      }),
    );
    expect(await amountOf(acc.id, "BTC")).toBe(4);

    // 插在两者之间,把量按到 100 —— 之后那笔 reduce 6 扣的是 100。
    await call(
      USER,
      handleCreateManualActivities({
        accountId: acc.id,
        drafts: [draft("BTC", "set", 100, at(7))],
      }),
    );

    expect(await amountOf(acc.id, "BTC")).toBe(94);
  });
