...
describe("listPortfolioMemberships", () => {
  ...
  it("别人的归属关系不出现在我的清单里", async () => {
    const theirDef = await db(otherUser(USER)).portfolios.ensureDefault();
    const theirAcc = await account(otherUser(USER), "他们的");
    await db(otherUser(USER)).portfolios.assignAccount(theirAcc.id, theirDef.id);
    await db(USER).portfolios.ensureDefault();
