...
describe("moveAccountToPortfolio", () => {
  ...
  it("移到它已经在的那个 → 幂等,不是报错", async () => {
    ...
    await call(USER, handleMoveAccountToPortfolio({ accountId: account.id, portfolioId: def.id }));
  ...
  it("要移的账户是别人的 → NotFound,对方的归属不动", async () => {
    const { target } = await seed(USER);
    const theirs = await seed(otherUser(USER));

    const exit = await callExit(
      USER,
      handleMoveAccountToPortfolio({ accountId: theirs.account.id, portfolioId: target.id }),
    );

    expect(failureOf(exit)).toBeInstanceOf(NotFound);
    expect(await whereIs(otherUser(USER), theirs.account.id)).toBe(theirs.def.id);
  });

  it("账户不存在 → NotFound", async () => {
    const { target } = await seed(USER);

    const exit = await callExit(
      USER,
      handleMoveAccountToPortfolio({ accountId: "没有这个", portfolioId: target.id }),
    );
