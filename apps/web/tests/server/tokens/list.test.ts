...
describe("listTokens(搜长尾)", () => {
  ...
  it("点中不建行 —— 搜十次也不留一行垃圾", async () => {
    ...
    for (const q of ["b", "bt", "btc", "btc ", "bitc", "bitco", "bitcoi", "bitcoin", "btcx", "x"]) {
  ...
  it("搜一个不存在的词 → 空结果,不是报错", async () => {
    stubOutbound([["/search", () => json({ coins: [] })]]);

    expect(await call(USER, handleListTokens({ query: "根本没有这个币" }))).toEqual([]);
  });
