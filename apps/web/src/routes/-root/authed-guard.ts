...
// 再被 JS 弹走。根路由是仍然在服务端跑的最后一层,判据换成「cookie 在不在」——
// better-auth's own `getSessionCookie`: instance-free (constructs nothing, touches no
// env/DB), handles the `__Secure-` prefix and both `.`/`-` name separators, treats an
// empty value as absent; no decrypt, no DB round-trip, microseconds.
//
...
const sessionCookiePresent = createIsomorphicFn()
  .server(() => getSessionCookie(getRequestHeaders()) !== null)
  .client(() => true);
...
// 自动在内。`satisfies keyof FileRoutesById` pins this literal to the generated route
// table: renaming `_authed` without updating this line turns typecheck red instead of
// silently disabling the server-side 307.
const AUTHED_ROUTE_ID = "/_authed" satisfies keyof FileRoutesById;
