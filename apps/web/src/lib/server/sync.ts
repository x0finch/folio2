...
export const triggerSync = authedServerFn({ method: "POST" }).handler(({ context }) => {
  const deps: SyncDeps = {
    listAccounts: (userId) => listAccountsByUser(env, userId),
    getEncryptedCredentials: (userId, accountId) => getEncryptedCredentials(env, userId, accountId),
