...
export const searchTokens = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const out = await buildOracle(env).tokens.search(q);
...
export const topTokens = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const out = await buildOracle(env).tokens.topTokens(limit);
...
export const refreshStalePrices = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const refreshed = await buildOracle(env).tokens.refreshStalePrices(assets);
