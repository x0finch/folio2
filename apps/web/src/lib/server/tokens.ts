...
export const searchCoins = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!q) return [];
    try {
      const out = await buildTokens(env).provider.searchCoins(q);
...
export const topCoins = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const top = await tokens.store.listTopTokens(limit);
    ...
    try {
      ...
      warmInFlight ??= tokens.refreshWarm({ now: Date.now() }).finally(() => {
        warmInFlight = null;
      });
      await warmInFlight;
    } catch (err) {
    ...
    const after = await tokens.store.listTopTokens(limit);
...
export const coinPrice = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const tokens = buildTokens(env);
    const ref: TokenRef = { source: tokens.provider.source, coinId: data.coinId as CoinId };
    tokenLog.debug("coinPrice: enter", { coinId: data.coinId });
    try {
      const cached = (await tokens.store.getPrices([ref])).get(refKey(ref));
      const hit = cached ?? (await fetchAndCachePrice(tokens, ref));
...
async function fetchAndCachePrice(tokens: Tokens, ref: TokenRef) {
  const p = (await tokens.provider.fetchPrices([ref])).get(refKey(ref));
  if (p) await tokens.store.putPrices([p], PRICE_TTL_MS);
  return p;
}

// 组装 tokens 实例:默认 provider(CoinGecko)+ D1 store(按 provider.source 分桶)。
...
export async function enrichBalances<T extends BalanceLike>(
  ...
  const refs = await Promise.all(
    balances.map(async (b) => {
      ...
      return (await tokens.resolveAsset(asset, { lazy: false })).ref;
    }),
  );
  const present = refs.filter((r) => r !== null);
  const [infos, prices] = await Promise.all([
    tokens.store.getInfo(present),
    tokens.store.getPrices(present),
  ]);
...
export async function warmTokens(
  ...
  try {
    await tokens.refreshWarm({ now });
    for (const b of balances) {
      const asset = balanceToAssetRef(b);
      if (asset) await tokens.resolveAsset(asset, { lazy: true });
    }
  } catch {
