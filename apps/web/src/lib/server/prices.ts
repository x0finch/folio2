...
import { refreshableTokenIds, userDisplayBalances } from "../core/tokens";
  ...
  .handler(async ({ context }) => {
    ...
    const report = await runRequest(
      ...
      Effect.gen(function* () {
        ...
        // 两次读互不依赖 → 并发取。
        const [snapshots, accounts] = yield* Effect.all(
          [snapshotStore.latest(), accountStore.list()],
          { concurrency: 2 },
        );
        ...
        return yield* Effect.flatMap(TokenService, (tokens) => tokens.refreshStale(ids));
      }),
    );
