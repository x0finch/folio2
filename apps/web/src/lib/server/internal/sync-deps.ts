...
// 判据就是 ref 的形状(`contract:` 标记,ADR 0020 第三轮)—— 不再问平台、不再走 connector 级联。
// logo/name 只在取数瞬时存在,不落快照行 —— 参考层是其 home。
function toProviderAssets(rows: Balance[]): ProviderAsset[] {
  const out: ProviderAsset[] = [];
  for (const b of rows) {
    // 只 seed 合约形:原生币走 symbol、`coingecko/<id>` 已是规范 ref、场馆命名走 symbol 消歧。
    if (parseTokenRef(b.tokenRef).kind !== "contract") continue;
