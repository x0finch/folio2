...
// 链键(<slug> / evm:<id>)不是 connectorId → getConnector 返回 undefined → null → 走 platforms 逐链查。
...
export function connectorPlatformMeta(
  ...
  // 平台键即 connectorId(binance/okx/hyperliquid/manual),或链键(短形:`bitcoin`/`solana`/`evm:<id>`)。
