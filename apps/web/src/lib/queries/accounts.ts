...
//
// **列表与持仓这一片不设 `staleTime`(即 0)。** 账户的写路径还没迁(#414),它们仍靠整页
// `router.invalidate()`,而那条路只在数据 stale 时才真拉 —— 现在开缓存,加账户 / 改凭据就会静默地不刷新。
//
...
export const accountListQuery = () =>
  queryOptions({ queryKey: accountKeys.list(), queryFn: () => listAccounts() });

export const accountHoldingsQuery = () =>
  queryOptions({ queryKey: accountKeys.holdings(), queryFn: () => listAccountHoldings() });
