...
// 缝③ 纯逻辑(无 server/db import → 可单测):server 只负责喂 enrich 结果 + fx 的 resolve。
//
// 两条支(身份**以 ref 为准**,不看裸 symbol —— "USD" 这种符号可能撞普通代币):
//  · **法币**(ref 是白名单 `fiat/issued:<CODE>`,由 `fiatCodeOf` 判定,ADR 0025):价 = FX 汇率
//    (`usd_per_unit`,USD 恒 1),**每次现算不冻价** —— 汇率变则非美元法币的 USD 值随之变,与
//    mark-to-market 一致。汇率缺(非美元且缓存冷)→ 该项 `undefined` → `buildManualSnapshot` 回退
...
export async function manualUnitPrices(
  ...
  return Promise.all(
    tokens.map((t) => {
      const fiatCode = t.ref ? fiatCodeOf(t.ref) : undefined;
      if (fiatCode) {
