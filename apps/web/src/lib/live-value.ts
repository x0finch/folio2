...
// 代币能力不再当参数传,而是 `R` 通道上的 `TokenReader` —— 调用方(server fn)一次 `runOracle` 全供上。
...
export const deriveLiveAccountTotals = (
  ...
  mode: ValuationMode,
): Effect.Effect<Map<string, number>, never, TokenReader> =>
  Effect.gen(function* () {
    ...
    const enriched = yield* Effect.flatMap(TokenReader, (t) => t.enrich(ids));
