...
//   · 迁移后的新 4-kind:spot / defi / perp_equity / perp_position
//   · 或旧 provider 写的遗留 kind:manual(→spot)/ perp(靠 meta.role)/ 旧 utxo(BTC,→spot)
// 本助手把两者归到统一 ViewKind,读端只认 4-kind。**未知 kind 兜底 spot、绝不 throw**
...
export function viewKind(row: KindRow): ViewKind {
  switch (row.kind) {
    ...
    case "utxo": // 遗留:旧 BTC 快照(kind:"utxo")→ 现货(BTC 明细现走 markdown detail)
      return "spot";
    case "defi":
    case "perp_equity":
    case "perp_position":
...
// 进"首屏跨账户聚合"的同质现货口径:现货(含并入的 BTC)。defi/perp 不进聚合(走次级分区)。
